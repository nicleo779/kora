...
final class MapperImplClassGenerator {
    ...
    private static final String GENERATED_MAPPER_SUPPORT = "org/byteora/kyra/orm/runtime/GeneratedMapperSupport";
    private static final String SQL_NODES = "org/byteora/kyra/orm/dynamic/SqlNodes";
    ...
    byte[] buildMapperImplClass(String generatedQualifiedName,
                                ...
                                List<KyraOrmProcessor.MapperMethodSpec> mapperMethods,
                                List<KyraOrmProcessor.MapperCapabilityDelegateSpec> capabilityDelegates) {
        String classInternalName = AsmUtils.internalName(generatedQualifiedName);
        String mapperInternalName = AsmUtils.internalName(mapperType);
        ...
        for (KyraOrmProcessor.MapperCapabilityDelegateSpec delegate : capabilityDelegates) {
        ...
        for (KyraOrmProcessor.MapperMethodSpec mapperMethod : mapperMethods) {
        ...
        writeConstructor(cw, classInternalName, capabilityDelegates);
        for (KyraOrmProcessor.MapperMethodSpec mapperMethod : mapperMethods) {
            writeMapperMethod(cw, classInternalName, mapperType, mapperMethod);
        }
        for (KyraOrmProcessor.MapperCapabilityDelegateSpec delegate : capabilityDelegates) {
            for (KyraOrmProcessor.MapperCapabilityMethodSpec method : delegate.methods()) {
    ...
    private void writeClinit(ClassWriter cw, String classInternalName, List<KyraOrmProcessor.MapperMethodSpec> mapperMethods) {
        ...
        for (KyraOrmProcessor.MapperMethodSpec mapperMethod : mapperMethods) {
            ...
            emitDynamicSqlNode(mv, mapperMethod.statement().rootSqlNode(), true, true);
    ...
    private void writeMapperMethod(ClassWriter cw,
                                   String classInternalName,
                                   TypeElement mapperType,
                                   KyraOrmProcessor.MapperMethodSpec mapperMethod) {
        ...
        if (statement.commandType() == SqlCommandType.SELECT) {
            ...
            mv.visitFieldInsn(Opcodes.GETSTATIC, "org/byteora/kyra/orm/xml/SqlCommandType", statement.commandType().name(), "Lorg/byteora/kyra/orm/xml/SqlCommandType;");
            mv.visitFieldInsn(Opcodes.GETSTATIC, classInternalName, context.statementFieldName(statement.id()), DYNAMIC_SQL_NODE_DESC);
            mv.visitFieldInsn(Opcodes.GETSTATIC, classInternalName, methodParameterNamesFieldName(statement.id()), "[Ljava/lang/String;");
            mv.visitVarInsn(Opcodes.ALOAD, valuesLocal);
    ...
    private void writeCapabilityMethod(ClassWriter cw,
                                       String classInternalName,
                                       KyraOrmProcessor.MapperCapabilityDelegateSpec delegate,
                                       KyraOrmProcessor.MapperCapabilityMethodSpec method) {
    ...
    private void pushParameterValuesArray(MethodVisitor mv, String classInternalName, List<? extends VariableElement> parameters) {
        if (parameters.isEmpty()) {
            mv.visitFieldInsn(Opcodes.GETSTATIC, GENERATED_MAPPER_SUPPORT, "NO_PARAMETER_VALUES", OBJECT_ARRAY_DESC);
            return;
        }
    ...
    private void emitMethodAnnotationArray(MethodVisitor mv, List<? extends AnnotationMirror> annotations, String classInternalName) {
        ...
        if (includedAnnotations.isEmpty()) {
            mv.visitFieldInsn(Opcodes.GETSTATIC, GENERATED_MAPPER_SUPPORT, "NO_ANNOTATIONS", "[" + ANNOTATION_META_DESC);
            return;
        }
    ...
    private void emitDynamicSqlNode(MethodVisitor mv, DynamicSqlNode node) {
        emitDynamicSqlNode(mv, node, false, false);
    }

    private void emitDynamicSqlNode(MethodVisitor mv, DynamicSqlNode node, boolean trimLeading, boolean trimTrailing) {
        if (node instanceof TextSqlNode textSqlNode) {
            mv.visitLdcInsn(normalizeSqlText(textSqlNode.getText(), trimLeading, trimTrailing));
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "text", "(Ljava/lang/String;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof MixedSqlNode mixedSqlNode) {
            int childCount = mixedSqlNode.getChildren().size();
            AsmUtils.pushInt(mv, childCount);
            mv.visitTypeInsn(Opcodes.ANEWARRAY, "org/byteora/kyra/orm/dynamic/DynamicSqlNode");
            for (int i = 0; i < childCount; i++) {
                mv.visitInsn(Opcodes.DUP);
                AsmUtils.pushInt(mv, i);
                emitDynamicSqlNode(mv, mixedSqlNode.getChildren().get(i),
                        trimLeading && i == 0, trimTrailing && i == childCount - 1);
                mv.visitInsn(Opcodes.AASTORE);
            }
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "mixed", "([Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof IfSqlNode ifSqlNode) {
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "ifNode",
                    "(Ljava/lang/String;Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof TrimSqlNode trimSqlNode) {
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "trim",
                    "(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;[Ljava/lang/String;Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof ForEachSqlNode forEachSqlNode) {
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "foreach",
                    "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof ChooseSqlNode chooseSqlNode) {
            AsmUtils.pushInt(mv, chooseSqlNode.getWhenNodes().size());
            mv.visitTypeInsn(Opcodes.ANEWARRAY, "org/byteora/kyra/orm/dynamic/WhenSqlNode");
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "choose",
                    "([Lorg/byteora/kyra/orm/dynamic/WhenSqlNode;Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
        if (node instanceof BindSqlNode bindSqlNode) {
            ...
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "bind",
                    "(Ljava/lang/String;Ljava/lang/String;)Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;", false);
            return;
        }
    ...
    private void emitWhenNode(MethodVisitor mv, WhenSqlNode whenSqlNode) {
        ...
        mv.visitMethodInsn(Opcodes.INVOKESTATIC, SQL_NODES, "when",
                "(Ljava/lang/String;Lorg/byteora/kyra/orm/dynamic/DynamicSqlNode;)Lorg/byteora/kyra/orm/dynamic/WhenSqlNode;", false);
    }
    ...
    /**
     * Shrinks the SQL string constants embedded in the generated mapper:
     * <ul>
     *     <li>collapses every run of whitespace (newlines and indentation from the XML source) into
     *     a single space;</li>
     *     <li>strips {@code --} line comments and {@code /* *}{@code /} block comments, while keeping
     *     MySQL conditional comments ({@code /*!}) and optimizer hints ({@code /*+});</li>
     *     <li>optionally trims the leading/trailing space when the fragment is at the very start/end
     *     of the statement.</li>
     * </ul>
     * A single space is preserved for each collapsed run (except the trimmed outer ends) so token
     * boundaries and the gaps between adjacent dynamic SQL fragments stay intact. Content inside
     * single-quoted string literals (including {@code ''} escapes) is left untouched, and {@code #{}}
     * placeholders are unaffected.
     */
    static String normalizeSqlText(String text, boolean trimLeading, boolean trimTrailing) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (ch == '/' && i + 1 < length && text.charAt(i + 1) == '*'
                    && !(i + 2 < length && (text.charAt(i + 2) == '!' || text.charAt(i + 2) == '+'))) {
                int end = text.indexOf("*/", i + 2);
                if (end < 0) {
                    if (pendingSpace && (started || !trimLeading)) {
                        result.append(' ');
                    }
